...
def get_git_repo(
    ...
    assert (
        not outputDir.exists()
    ), f"Git clone output set to `{outputDir.resolve()}`, but `{outputDir.resolve()}` already exists."
    cmd_options = []
    if tag != None:
        cmd_options.extend(["-b", tag])
    cmd = ["git", "clone"]
...
if __name__ == "__main__":
    ...
    # `mkdir ~checkout`
    # `cd ~checkout`
    # `git clone <repo url> -b <tag>`, repeat this for each repo listed here
